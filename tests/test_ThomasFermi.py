...
def test_ThomasFermiSolver():
    ...
    PVs_over_kTZ       = [0.5227, 0.7028, 0.5210, 0.6470, 0.8283, 0.5266, 0.9936, 0.6239 , 0.9172 , 0.8037  , 0.2380 ]
    ...
    for V_Angstrom_cubed, T_keV, PV_over_kTZ, alpha_ref, b_ref, beta_boundary_ref in zip(
        ...
        # Epot > 0 since V(r) = T*beta/s - mu > 0 inside the ion sphere
        # and U includes the Epot contribution via virial theorem
        assert res['U'] > 1.5 * res['P']
